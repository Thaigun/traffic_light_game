...
class Game {
  ...
  var cars: Vector[Car] = Vector()
  var cossings: Vector[Crossing] = Vector()
  
  /*
   * The gameFile is not passed to the fileReader as an argument, because we might want to re-init the game with the same map. In
  ...
  def setRoads(roads: Array[Road]) = {
    
  }
  
  def setCrossings(crossings: Array[Crossing]) = {
